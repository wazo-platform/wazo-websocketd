...
from .helpers.wait_strategy import WaitUntilValidConnection
...
class TestBroker(IntegrationTest):
    ...
    def _broker_cache_size(self):
        # the broker reports its cached token count as `connections`, and other
        # sessions cache their own tokens, so only a decrease is meaningful
        return self.asset_cls.read_children_statuses()['broker']['connections']
    ...
    def _restore_broker(self):
        ...
        WaitUntilValidConnection().wait(self.asset_cls)
